...
socket.on("get-playerData", (uid, streak, corAns, points) => {
    console.log(`User ${uid} data updated: Answered correctly ${corAns}, current streak ${streak}, got ${points}`);
})
...
socket.on("get-stop", () => {
    console.log(`Forced to stop the game, need to submit and end the game immediately`);
})
